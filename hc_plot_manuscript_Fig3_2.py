...
#for j, (item, cmap, vmin, vmax) in enumerate(zip([phase,  srII, mag], [cmap1, cmap3, cmap4], [vmin_cmap1,  vmin_cmap3, vmin_cmap4], [vmax_cmap1,  vmax_cmap3, vmax_cmap4])):
    ...
    if j == 0:  # Assuming j == 1 corresponds to srII plot
        # Create logarithmically spaced contour levels
        log_levels = np.logspace(-5, -1, num=5)  # levels: 10^-3, 10^-2, 10^-1
        pls_contour = ax.contour(mid_x, mid_z, pls, levels=log_levels, colors='red', linewidths=2)
        manual_locations = [(530, -60), (440, -90), (560, -55)]
        # Add labels to the contours with appropriate formatting for logarithmic values
        ax.clabel(pls_contour, inline=False, manual=manual_locations, fontsize=25,  colors='black', fmt=lambda x: f'$10^{{{int(np.log10(x))}}}$')
    ...
    '''
    label = f"{text_labels[j+3]}"  # 'j' controls the letter and 'i' controls the number
...
cbar3 = fig.colorbar(caxes[0], cax=cbar_ax3, orientation='horizontal', fraction=0.1, pad=0.1)
...
cbar4 = fig.colorbar(caxes[1], cax=cbar_ax4, orientation='horizontal', fraction=0.1, pad=0.1)
...
plt.tight_layout()
#plt.subplots_adjust(left=0.05, right=0.98, bottom=0.07, top=0.8, wspace=0.2, hspace=1)
#plt.subplots_adjust(left=0.05, right=0.95, top=0.95, bottom=0.05, wspace=0.2, hspace=0.2)  # Adjust margins as needed
